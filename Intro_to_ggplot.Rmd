...
```{r mpg green, fig.height = 4}
ggplot(data = mpg) + 
  geom_point(mapping = aes(x = displ, y = hwy), color = "darkgreen")
```
  
Here, the color doesn't convey information about a variable, but only changes the appearance of the plot. To set an aesthetic manually, set the aesthetic by name as an argument of your geom function; i.e. it goes outside of `aes()`. You can use aesthetics for colour, shape, and size.
...
```{r geom_line, message = FALSE, fig.height = 4}
ggplot(data = mpg) +
...
```{r without se, message = FALSE, fig.height = 4}
ggplot(data = mpg) + 
...
```{r combined trend, echo = FALSE, message  = FALSE}
...
```{r multiple geoms, message = FALSE, , fig.height = 4}
ggplot(data = mpg) + 
...
```{r nice multiple geoms, eval = FALSE}
...
```{r overwrite layers, message = FALSE, fig.height = 4}
ggplot(data = mpg, mapping = aes(x = displ, y = hwy)) + 
